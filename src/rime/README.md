...
flowchart TD
    ...
    C --> D["Decapsulator in transformers/decapsulators<br/>DecapsulatedMessage(sensor_payloads, envelope_metadata)"]
    D --> E["Sensor registry lookup<br/>sensor_uuid to SupportedSensors"]
    E --> F["INGEST_COMPONENT_MAP in transformers/ingest_registry.py"]
    F --> G["Parser -> ParsedMessage"]
...
| Parse | [`transformers/parsers/`](transformers/parsers/README.md), [`transformers/messages.py`](transformers/messages.py) | Normalizes body, resolves timestamps; produces `ParsedMessage`. |
| STA normalization | [`transformers/normalizers/`](transformers/normalizers/README.md) | Maps `ParsedMessage.body` fields → `ObservedProperties` → `Observation` tuples. |
...
2. Provider decapsulates the payload into a `DecapsulatedMessage` (one `IdentifiedPayload` per sensor).
...
5. rime runs `parser.parse → ParsedMessage → normalizer.from_parsed → to_stObservations`.
