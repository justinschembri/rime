...
def _push_available(
    ...
    """Start the full rime stack via deploy compose scripts."""
    compose_dir = _compose_dir()
    start_script = compose_dir / "start-production.sh"
    ...
    if result.returncode != 0:
        ...
        if private:
            console.print("System starting with authentication at http://localhost:8080/rime")
        else:
            console.print("System starting (public access) at http://localhost:8080/rime")


def _stop_instance():
    """Stop the full rime stack via deploy compose scripts."""
    compose_dir = _compose_dir()
    stop_script = compose_dir / "stop-production.sh"
